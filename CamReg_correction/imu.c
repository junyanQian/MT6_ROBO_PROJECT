...
uint8_t show_direction(imu_msg_t *imu_values){
    ...
    */
    ...
    33 *              BACK
    34 */
    float *acc = imu_values->acceleration;
    if (acc[Y_AXIS] <= -THRESHOLD)
    {
    	if (acc[X_AXIS] >= -THRESHOLD){
    		return LEFT;
    	}else{
    		return RIGHT;
    	}
    }else if (acc[Y_AXIS] >= THRESHOLD)
    {
    	if (acc[X_AXIS] >= THRESHOLD){
    		return LEFT;
    	}else if (acc[X_AXIS] <= -THRESHOLD){
    		return RIGHT;
    	}else{
    		return FRONT;
    	}
